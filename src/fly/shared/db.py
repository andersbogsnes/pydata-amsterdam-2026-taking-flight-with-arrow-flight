...
def _upload_message_to_db(
    ...
    with local_file_path.open("rb") as f, engine.begin() as conn:
        ...
        with (
            ...
            cursor.copy(
                f"COPY {table_name} (ride_id, rideable_type, started_at, ended_at, start_station_name, start_station_id, end_station_name, end_station_id, start_lat, start_lng, end_lat, end_lng, member_casual) FROM STDIN (FORMAT CSV)"
            ) as copy,
        ):
